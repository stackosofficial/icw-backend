...
const isPhoneNo = (str: string) => {
  ...
  try {
    ...
    if (num > 9999999999 || num < 1000000000) return false;
...
export class EventsService {
  ...
  validateEvent = (event: Event) => {
    ...
    if (event.from) {
      const aTime = new Date(event.from);
      ...
      for (const key in updatedEvents) {
        if (updatedEvents[key].status == 'A') {
          ...
          if (
            ...
            approvedEvent[0].createdByEmail
          ) {
            try {
              await this.mailService.sendMail({
        ...
        if (
          ...
          try {
            await this.mailService.sendMail({
              ...
              from: process.env.EMAIL_USER,
              subject:
                deletedEvent[0].status == 'A'
                  ? 'Your event has been deleted.'
                  : 'Your event has been rejected.',
              text: `Your event ${deletedEvent[0].name} is ${
                deletedEvent[0].status == 'A' ? 'deleted' : 'rejected'
  ...
  async userAddEvent(event: Event): Promise<Event> {
    ...
    if (!resp.success) throw new Error(resp.reason);
  ...
  async adminChangeEvents(eventChanges) {
    ...
    if (eventChanges.deletedEvents) {
      for (let i = 0; i < eventChanges.deletedEvents.length; i++) {
  ...
  async getApprovedEvents(): Promise<any> {
    return await this.eventModel.find({ status: 'A' }).select({
      _id: 0,
      name: 1,
      link: 1,
      venue: 1,
