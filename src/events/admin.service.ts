...
export class AdminService {
    ...
    async generateToken(admin: Admin, force: boolean)
    {
        ...
        if(force || (!admin.baseToken) || (admin.baseToken && nowTime > admin.tokenTime)) {
        ...
        var token = AES.encrypt(revBase, admin.password).toString();
    ...
    async onModuleInit() {
        ...
        if(!admin) {
            ...
            await this.adminModel.updateOne({username: 'admin'}, {password: process.env.ADMIN_PASS});
        }
    }
}